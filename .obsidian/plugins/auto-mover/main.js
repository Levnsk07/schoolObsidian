...
var SettingsTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin) {
    ...
    this.display = () => {
      ...
      containerEl.empty();
      new import_obsidian2.Setting(containerEl).setName("Automatic moving").setHeading();
      new import_obsidian2.Setting(containerEl).setName("Export/Import of settings").setDesc(
      ...
      new import_obsidian2.Setting(containerEl).setName("Move on open").setDesc("Should the file be moved when it is opened?").addToggle(
        (cb) => cb.setValue(this.plugin.settings.moveOnOpen).onChange(async (value) => {
          this.plugin.settings.moveOnOpen = value;
          await this.plugin.saveData(this.plugin.settings);
        })
      );
      new import_obsidian2.Setting(containerEl).setName("Manually move").setDesc("Execute the command to go through your notes and move them according to the rules specified below.").addButton((button) => {
      ...
      new import_obsidian2.Setting(automaticMovingContainer).setName("Automatic moving").setDesc(
...
var MovingUtil = class {
   ...
   */
  isFolder(path) {
    return this.app.vault.getAbstractFileByPath(path) instanceof import_obsidian3.TFolder;
  }
  /**
...
Creating requested folder.`
  ...
  moveFolder(folder, newPath) {
    if (this.isFolder(newPath)) {
      ...
      new import_obsidian3.Notice(`Invalid destination path
...
${newPath} is not a folder!`);
  ...
  createFolder(path) {
    if (!this.isFolder(path)) {
      ...
      new import_obsidian3.Notice("Folder already exists", 5e3);
...
var AutoMoverPlugin = class extends obsidian.Plugin {
  ...
  goThroughAllFiles() {
    ...
    new obsidian.Notice("All files moved!", 5e3);
  }
  /**
  ...
  matchAndMoveFile(file) {
    if (this.matchAndMoveByProject(file))
      return;
    else if (this.matchAndMoveByName(file))
      return;
    else
      this.matchAndMoveByTag(file);
  }
  /**
  ...
  matchAndMoveByTag(file) {
    ...
    if (RuleMatcherUtil_default.isRegexGrouped(tagRule)) {
      ...
      MovingUtil_default.moveFile(file, tagRule.folder);
    }
    return true;
  }
  matchAndMoveByProject(file) {
    const cache = this.app.metadataCache.getFileCache(file);
    if (cache == null)
      return false;
    ...
    if (projectRule.rules == null || projectRule.rules.length === 0) {
      ...
      MovingUtil_default.moveFile(file, projectRule.folder);
      return true;
    }
    ...
    if (RuleMatcherUtil_default.isRegexGrouped(rule)) {
      ...
      const finalDestinationPath = ProjectMatcherUtil_default.constructProjectDestinationPath(projectRule, ruleDesinationPath);
      MovingUtil_default.moveFile(file, finalDestinationPath);
    } else {
